...
private:
	...
	{
		int child = 0;
		T temp = this->_items[hole];	

		for (; hole * 2 <= this->size(); hole = child) {
			child = hole * 2;
			if (child != this->size() && this->_items[child + 1] < this->_items[child]) {
				child++;
			}
	...
	void percolateUp(T item)
	{
		...
		for (; hole > 1 && item < this->_items[hole / 2]; hole /= 2) {
			this->_items[hole] = this->_items[hole / 2];					//Swapping nodes until item is greater than parent
		}
		
		this->_items[hole] = item;			//At correct position for insert, insert item 
